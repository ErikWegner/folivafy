    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
    ...
    async fn search_collection(
        &self,
        param_collection: String,
        param_search_collection_body: models::SearchCollectionBody,
        param_extra_fields: Option<String>,
        param_limit: Option<i32>,
        param_offset: Option<i32>,
        param_sort: Option<String>,
        context: &C) -> Result<SearchCollectionResponse, ApiError>
    {
        let mut client_service = self.client_service.clone();
        let mut uri = format!(
            "{}/api/collections/{collection}/searches",
            self.base_path
            ,collection=utf8_percent_encode(&param_collection.to_string(), ID_ENCODE_SET)
        );
        ...
        let body = serde_json::to_string(&param_search_collection_body).expect("impossible to fail to serialize");
                *request.body_mut() = Body::from(body);
        ...
        match response.status().as_u16() {
            200 => {
                ...
                Ok(SearchCollectionResponse::SuccessfulOperation
                    (body)
                )
            }
            404 => {
